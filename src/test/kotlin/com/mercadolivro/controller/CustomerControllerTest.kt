...
class CustomerControllerTest {
    ...
    @Test
    fun `should return all customers when filter by name`() {
        val customer1 = customerRepository.save(buildCustomer(name = "Julio"))
        customerRepository.save(buildCustomer(name = "Jose"))

        mockMvc.perform(get("/customers?name=Jul"))
            .andExpect(status().isOk)
    ...
    @Test
    fun `should return user get by id when the same id`() {
    ...
    @Test
    fun `should return forbidden when user different id`() {
        val customer = customerRepository.save(buildCustomer())
    ...
    fun `should return user get by id when admin`() {
        val customer = customerRepository.save(buildCustomer())
    ...
    fun `should update customer`() {
        val customer = customerRepository.save(buildCustomer())
        val request = PutCustomerRequest(name = "Julio", email = "update@email.com")
    ...
    fun `should return not found when update customer not exist`() {
        val request = PutCustomerRequest("fake", "${Random.nextInt()}@email.com")
    ...
    @Test
    fun `should return not found when customer not exist`() {
